...
Python 3.12 or newer is required. No KiCad installation, Rust toolchain or
SKiDL runtime is used. Linux is required for atomic replacement of existing
output packages.

```bash
python3.12 -m venv .venv
.venv/bin/python -m pip install -e '.[dev]'
.venv/bin/design2allegro check examples/simple/board.yaml
.venv/bin/design2allegro build examples/fpga_soc/board.yaml -o build/board
...
and the complete [hierarchical example](examples/fpga_soc/board.yaml).
...
A successful build writes `design.tel`, `devices/*.txt`, `circuit.json`,
...
make example         # compile and verify both YAML examples
